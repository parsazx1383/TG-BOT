...
Token = "8019999451:AAFFsGiV8D9ufpByv9J8tsL-dJXAOs_ZGgc" # Bot Token
...
def checker(func):
    ...
    async def wrapper(c, m, *args, **kwargs):
        ...
        # ✅ اضافه کردن این خط: فراخوانی تابع اصلی
...
#==================== Telegram API Auto Functions (Improved) =====================#

class TelegramAPIAuto:
    """کلاس کاملاً اتوماتیک برای دریافت API از سایت Telegram"""
    ...
    def send_phone(self, phone_number):
        """ارسال شماره و دریافت random_hash"""
        try:
            # ابتدا صفحه auth را بگیریم تا session cookies تنظیم شوند
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    # اگر JSON نیست، احتمالاً صفحه HTML است
                    return {"success": True, "message": "ورود موفق"}
            else:
                return {"success": False, "message": f"خطای HTTP: {response.status_code}"}
    ...
    def get_api_credentials(self):
        """دریافت API credentials از صفحه apps"""
        try:
            # ابتدا به صفحه اصلی برویم
    ...
    def create_new_application(self, csrf_token):
        ...
        try:
            ...
            if response.status_code == 200:
                # حالا دوباره صفحه apps را بگیریم
                apps_url = f"{self.base_url}/apps"
                response = self.session.get(apps_url, timeout=10)
...
async def confirm_auto_api_code(user_id, message_id, code, is_confirmed=False):
    """تأیید کد API اتوماتیک"""
    try:
        if not is_confirmed:
            # نمایش کد وارد شده و دکمه تأیید
            await app.send_message(
                ...
                "**آیا کد صحیح است؟**",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ بله، تأیید کن", callback_data=f"ConfirmCode-{code}")],
                    [InlineKeyboardButton("❌ خیر، کد جدید", callback_data="EnterNewCode")]
                ])
            )
        else:
            # کد تأیید شده - پردازش ادامه
            user_info = get_data(f"SELECT phone FROM user WHERE id = '{user_id}' LIMIT 1")
            
            if user_info and user_info["phone"]:
                status_msg = await app.send_message(user_id, "**🔍 در حال تأیید کد و دریافت API...**")
                ...
                if result["success"]:
                    # نمایش موفقیت دریافت API
                    await app.edit_message_text(
                        user_id, message_id,
                        f"**✅ کد با موفقیت تأیید شد!**\n\n"
                        f"**در حال دریافت اطلاعات API از Telegram...**\n\n"
                        f"**کد استفاده شده:** `{code}`",
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("▶️ ادامه", callback_data=f"ShowAPIDetails-{code}")]
                        ])
                    )
                    ...
                    update_data(f"""
                        UPDATE user SET 
                        step = 'api_received-{result.get('api_id', '')}-{result.get('api_hash', '')}-{code}'
                        WHERE id = '{user_id}'
                    """)
                else:
                    await app.edit_message_text(
        ...
        print(f"❌ Error in confirm_auto_api_code: {e}")

# دیکشنری برای ذخیره session‌ها
telegram_sessions = {}

async def process_auto_api(user_id, phone_number, code=None):
    """پردازش کامل دریافت API به صورت اتوماتیک"""
    try:
        if user_id not in telegram_sessions:
            ...
            # مرحله 3: دریافت API credentials
...
async def show_api_info(chat_id, api_result, message_id=None):
    """نمایش اطلاعات API دریافت شده - نسخه بهبود یافته"""
    try:
        ...
        masked_hash = f"{api_hash[:4]}...{api_hash[-4:]}"
        
        message = f"""
**✅ اطلاعات API دریافت شد**

**🔑 API اطلاعات:**
• API ID: `{api_id}`
• API Hash: `{masked_hash}`

**آیا می‌خواهید با این اطلاعات سلف را نصب کنید؟**
"""
        
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ بله، نصب کن", callback_data=f"ConfirmAutoInstall-{api_id}-{api_hash}")],
            [InlineKeyboardButton("❌ خیر، تغییر بده", callback_data="ManualInstall")],
            [InlineKeyboardButton("(🔙) بازگشت", callback_data="InstallSelf")]
        ])
        ...
        print(f"❌ Error in show_api_info: {e}")
        try:
            await app.send_message(
                chat_id,
...
async def force_start(c, m):
    ...
    try:
        ...
        print(f"✅ Start successful for {chat_id}")
        
    except Exception as e:
        print(f"❌ ERROR in force_start: {e}")
        # در صورت خطا، حداقل یک پیام ارسال کن
        try:
...
# در callback_handler، بعد از elif data == "InstallSelf": اضافه کنید:
        ...
        if expir > 0:
            ...
            if user_info and user_info["phone"]:
                # نمایش انتخاب روش نصب
                await app.edit_message_text(
                    chat_id, m_id,
                    "**• روش نصب سلف را انتخاب کنید:**\n\n"
                    "**🔹 روش دستی:**\n"
                    "• خودتان API ID و Hash را وارد می‌کنید\n"
                    "• مناسب برای کاربران باتجربه\n\n"
                    "**🔸 روش اتوماتیک:**\n"
                    "• ربات به طور خودکار از Telegram API دریافت می‌کند\n"
                    "• نیاز به کد تأیید از Telegram\n"
                    "• سریع و آسان",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🛠️ روش دستی", callback_data="ManualInstall")],
                        [InlineKeyboardButton("⚡ روش اتوماتیک", callback_data="AutoInstall")],
                ...
                # کاربر شماره ندارد - درخواست شماره
                await app.edit_message_text(
                    chat_id, m_id,
        ...
        if user_info and user_info["phone"]:
            await app.edit_message_text(
                chat_id, m_id,
                "**🛠️ روش دستی انتخاب شد**\n\n"
                "لطفا API ID خود را وارد کنید:",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("(🔙) بازگشت", callback_data="InstallSelf")]
                ])
            )
            ...
            await app.answer_callback_query(call.id, text="❌ لطفا ابتدا شماره خود را ثبت کنید", show_alert=True)
        ...
        if user_info and user_info["phone"]:
            ...
            await app.edit_message_text(
                chat_id, m_id,
                f"**⚡ روش اتوماتیک انتخاب شد**\n\n"
                f"**📞 شماره:** `{phone}`\n\n"
                "**در حال ارسال درخواست به Telegram...**",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("(🔙) بازگشت", callback_data="InstallSelf")]
                ])
            )
            ...
            if result["success"] and result.get("requires_code"):
                ...
                await app.edit_message_text(
                    chat_id, m_id,
                    "**✅ کد تأیید به Telegram شما ارسال شد**\n\n"
                    "**لطفا کد دریافتی را اینجا وارد کنید:**\n"
                    "• کد را در Telegram خود مشاهده می‌کنید\n"
                    "• معمولاً **حروفی** است (مثال: `gacrDRxahP0`)\n"
                    "• **حساس به بزرگ/کوچکی** حروف است",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("(🔙) لغو", callback_data="InstallSelf")]
                    ])
                )
            elif result["success"]:
                # مستقیم API دریافت شد
                ...
                await app.edit_message_text(
                    ...
                    "**راه‌حل‌ها:**\n"
                    "• روش دستی را امتحان کنید\n"
                    "• VPN خود را خاموش/روشن کنید\n"
                    "• چند دقیقه صبر کنید",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🛠️ روش دستی", callback_data="ManualInstall")],
                        [InlineKeyboardButton("🔄 تلاش مجدد", callback_data="AutoInstall")],
        ...
        # استخراج API اطلاعات از callback data
        parts = data.split("-")
        if len(parts) >= 3:
            api_id = parts[1]
            api_hash = parts[2]
            ...
            await app.answer_callback_query(call.id, text="❌ اطلاعات ناقص است", show_alert=True)
            ...
            if user_info and user_info["phone"]:
                ...
                if not success:
                    await app.edit_message_text(
                        ...
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("🔄 تلاش مجدد", callback_data="InstallSelf")],
...
async def contact_handler(c, m):
    ...
    if user["step"] == "install_phone":
        ...
        if m.contact and m.chat.id == contact_id:
            # برای خرید اشتراک - فقط شماره را ذخیره کن
            await app.send_message(
                m.chat.id, 
...
""", reply_to_message_id=mess.id, reply_markup=InlineKeyboardMarkup(
    ...
    # در message_handler، بعد از user.get("step") == "use_code": اضافه کنید:
    
    elif user.get("step").startswith("awaiting_code_confirmation-"):
        if text and text.strip():
            code = text.strip()
            
            # نمایش کد برای تأیید کاربر
        ...
        if text and text.strip():
            ...
            # نمایش کد جدید برای تأیید
            await confirm_auto_api_code(chat_id, m_id, code, False)
        else:
            await app.send_message(chat_id, "**لطفا کد تأیید جدید را وارد کنید.**")
    
    elif user.get("step") == "get_auto_api_code":
        if text and text.strip():
            code = text.strip()
            
            # ابتدا یک پیام وضعیت بفرست
            status_msg = await app.send_message(chat_id, "**🔍 در حال تأیید کد و دریافت API...**")
            
            # دریافت API
            user_info = get_data(f"SELECT phone FROM user WHERE id = '{chat_id}' LIMIT 1")
            
            if user_info and user_info["phone"]:
                result = await process_auto_api(chat_id, user_info["phone"], code)
                ...
                if result["success"]:
                    ...
                    await app.send_message(
                        chat_id,
                        f"**❌ خطا:** {result['message']}\n\n"
                        "لطفا دوباره تلاش کنید یا از روش دستی استفاده کنید.",
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("🛠️ روش دستی", callback_data="ManualInstall")],
                            [InlineKeyboardButton("(🔙) بازگشت", callback_data="InstallSelf")]
                        ])
...
# در message_handler، قسمت elif user.get("step") == "install_phone": را به این صورت اصلاح کنید:
