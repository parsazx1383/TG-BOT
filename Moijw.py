...
async def show_api_info(chat_id, api_result, original_message_id=None):
    """نمایش اطلاعات API دریافت شده - اصلاح شده"""
    try:
...
"""
        ...
        # همیشه پیام جدید بفرست (برای اطمینان)
        ...
        # اگر message_id معتبر داشتیم و هنوز پیام موجود است، حذفش کن
        if original_message_id:
            try:
                await app.delete_messages(chat_id, original_message_id)
            except:
                pass  # اگر نشد، مهم نیست
        
        # ذخیره API در دیتابیس
        update_data(f"""
            UPDATE user SET 
            api_id = '{api_id}',
            api_hash = '{api_hash}',
            step = 'auto_api_received'
            WHERE id = '{chat_id}'
        """)
        ...
        # حداقل یک پیام ساده ارسال کن
        try:
            await app.send_message(
                chat_id,
                f"**✅ API اطلاعات دریافت شد**\n"
                f"**API ID:** `{api_id}`\n"
                f"**API Hash:** `{masked_hash}`\n\n"
                f"لطفا از منوی اصلی ادامه دهید.",
            )
        except:
            pass
        ...
        if user_info and user_info["phone"]:
            ...
            if result["success"] and result.get("requires_code"):
                await app.edit_message_text(
                    ...
                    "• کد در Telegram شما نمایش داده می‌شود\n"
                ...
                update_data(f"UPDATE user SET step = 'get_auto_api_code' WHERE id = '{chat_id}'")
            elif result["success"]:
                # مستقیم API دریافت شد
                await show_api_info(chat_id, m_id, result)
            else:
                await app.edit_message_text(
                    chat_id, m_id,
