...
DBUser = "amyeyen_SEX" # Database User
...
class TelegramAPIAuto:
    ...
    def send_phone(self, phone_number):
        ...
        try:
            ...
            response = self.session.post(send_url, data=data, timeout=10)
    ...
    def get_api_credentials(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self.extract_api_credentials(soup)
            else:
    ...
    def extract_api_credentials(self, soup):
        """استخراج API credentials از صفحه"""
        try:
            # جستجوی در کل متن صفحه
            page_text = str(soup)
            
            # جستجوی API ID
            api_id_match = re.search(r'api_id[\s:]+([0-9]+)', page_text, re.IGNORECASE)
            api_id = api_id_match.group(1) if api_id_match else None
            
            # جستجوی API Hash
            api_hash_match = re.search(r'api_hash[\s:]+([a-f0-9]{32})', page_text, re.IGNORECASE)
            ...
            if api_id and api_hash:
                ...
                # اپلیکیشنی وجود ندارد، باید ایجاد کنیم
                return self.create_new_application(soup)
                
        except Exception as e:
            return {"success": False, "message": f"خطا در استخراج اطلاعات: {str(e)[:100]}"}
    
    def create_new_application(self, soup):
        """ایجاد اپلیکیشن جدید"""
        try:
            # ابتدا باید CSRF token را پیدا کنیم
            csrf_token = None
            ...
            if response.status_code == 200:
                # بعد از ایجاد، دوباره اطلاعات را بگیریم
                soup = BeautifulSoup(response.text, 'html.parser')
                return self.extract_api_credentials(soup)
            else:
...
async def show_api_info(chat_id, message_id, api_result):
    """نمایش اطلاعات API دریافت شده"""
...
# در callback_handler، بعد از elif data == "InstallSelf": اضافه کنید:
        ...
        if user_info and user_info["phone"]:
            ...
            await app.edit_message_text(
                ...
                f"**📞 شماره:** `{phone}`\n\n"
                "در حال ارسال درخواست به سایت Telegram.org...",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("(🔙) بازگشت", callback_data="InstallSelf")]
                ])
            )
            
            # شروع فرآیند اتوماتیک
            result = await start_auto_api_process(chat_id, phone)
            
            if result["success"] and result.get("requires_code"):
                await app.edit_message_text(
                    chat_id, m_id,
                    "**✅ درخواست به Telegram ارسال شد**\n\n"
...
""", reply_to_message_id=mess.id, reply_markup=InlineKeyboardMarkup(
    ...
    # در message_handler، بعد از user.get("step") == "use_code": اضافه کنید:
        ...
        if text and text.strip():
            ...
            await app.send_message(chat_id, "**🔍 در حال تأیید کد و دریافت API...**")
            
            result = await verify_and_get_api(chat_id, code)
