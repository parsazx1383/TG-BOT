...
Token = "7723792739:AAG08Ysz6OkYoAVrflry0pex07eTZjODKwE" # Bot Token
...
DBName = "TG_SELFSAZ" # Database Name
DBUser = "TG_SELFSAZ" # Database User
DBPass = "Zxcvbnm1111" # Database Password
HelperDBName = "TG_HELPER" # Helper Database Name
HelperDBUser = "TG_HELPER" # Helper Database User
...
لطفا عکس از کارتی که میخواهید با آن خرید انجام دهید ارسال کنید...**""",
    ...
    # در تابع callback_handler، بعد از هندلرهای موجود این موارد را اضافه کنید:
        ...
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            await app.edit_message_text(chat_id, m_id,
                                   "**لطفا تعداد روزهای انقضای کد را وارد کنید:**",
                                   reply_markup=InlineKeyboardMarkup([
                                       [InlineKeyboardButton(text="(🔙) بازگشت", callback_data="AdminPanel")]
                                   ]))
        ...
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            ...
            if codes:
                ...
                for code in codes:
                    keyboard_buttons.append([
                        InlineKeyboardButton(text=f"حذف کد: {code['code']}", callback_data=f"DeleteCode-{code['id']}")
                    ])
                ...
                await app.answer_callback_query(call.id, text="هیچ کد فعالی برای حذف وجود ندارد.", show_alert=True)

    elif data.startswith("DeleteCode-"):
        if call.from_user.id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{call.from_user.id}' LIMIT 1") is not None:
            code_id = data.split("-")[1]
            delete_code(code_id)
            await app.answer_callback_query(call.id, text="کد با موفقیت حذف شد.", show_alert=True)
            await app.edit_message_text(chat_id, m_id,
                                   "**کد با موفقیت حذف شد.**",
                                   reply_markup=InlineKeyboardMarkup([
                                       [InlineKeyboardButton(text="(🔙) بازگشت", callback_data="AdminPanel")]
    ...
    # در تابع message_handler، بعد از هندلرهای موجود این موارد را اضافه کنید:
        ...
        if chat_id == Admin or helper_getdata(f"SELECT * FROM adminlist WHERE id = '{chat_id}' LIMIT 1") is not None:
            if text.isdigit():
                ...
                await app.send_message(chat_id,
                                 f"**✅ کد با موفقیت ایجاد شد:**\n\n"
                                 f"**کد:** `{code}`\n"
                                 f"**روزهای انقضا:** {days} روز\n\n"
                                 f"**تاریخ ایجاد:** {time.strftime('%Y-%m-%d %H:%M:%S')}",
                                 reply_markup=InlineKeyboardMarkup([
                                     [InlineKeyboardButton(text="(🔙) بازگشت", callback_data="AdminPanel")]
                                 ]))
                update_data(f"UPDATE user SET step = 'none' WHERE id = '{chat_id}' LIMIT 1")
        ...
        if code_data:
            ...
            await app.send_message(chat_id, "**کد وارد شده نامعتبر است یا قبلاً استفاده شده است.**")
